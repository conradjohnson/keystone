...
function Profile(){
  ...
  const { loading, error, data } = useQuery( QUERY_USER);
    ...
    return (
        Auth.loggedIn() ? (
        <>
          <div className="flex flex-col pl-5 h-contain w-3/5 rounded shadow-lg bg-purple-100 ml-72">
        ...
        {/* <div>
          ...
          {user.properties &&
            user.properties.map((property) => (
              <div className="max-w-lg items-center overflow-hidden border-2 border-purple-800 bounce-top-cards snap-start my-4" key={property._id}>
